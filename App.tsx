...
interface FormData {
  ...
  formFocus: FormFocus; // Novo campo para controlar o fluxo de "Outros"
    ...
    <div className="flex items-center gap-3">
      <img 
        ...
        onError={(e) => {
            ...
            e.currentTarget.parentElement.querySelector('.fallback-logo').style.display = 'flex';
        }}
    ...
    <div className="min-h-[calc(100vh-80px)] relative flex flex-col items-center justify-center text-center px-4 overflow-hidden">
      <div className="absolute inset-0 z-0">
        <img 
          ...
          onError={(e) => e.currentTarget.src = 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?q=80&w=1600&auto=format&fit=crop'}
        />
...
const FeedbackPage = ({ onReset }: { onReset: () => void }) => {
  const handleShare = () => {
    // Nota para o TCC: Em produção, isso seria a URL real do site (ex: projeto-denuncia.vercel.app)
    ...
    if (navigator.share) {
      ...
      // Usando um alerta customizado ou simples
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] relative flex flex-col items-center justify-center px-4 bg-gray-50">
      ...
      <div className="relative z-10 max-w-md w-full bg-white p-8 rounded-2xl shadow-2xl text-center border-b-8 border-[#669935] animate-in fade-in zoom-in duration-500">
...
const FormPage = ({ onSubmitSuccess, onBack }: { onSubmitSuccess: () => void, onBack: () => void }) => {
  ...
  // Referência para scrollar até o erro
  ...
  const validateForm = () => {
    ...
    // 4. Perguntas da Seção (Obrigatórias?)
    // Para um TCC de segurança, geralmente todas as perguntas de Check são cruciais.
    // Vamos assumir que pelo menos 50% devem ser respondidas ou todas. Vamos forçar todas para garantir qualidade dos dados.
    if (isChildFlow) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setAgeTouched(true); // Força validação de idade visualmente

    if (!validateForm()) {
        // Scroll suave para o topo ou para o erro
    ...
    try {
        ...
        if (user) {
             await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'denuncias'), {
                ...
                status: 'new',
                deviceInfo: navigator.userAgent // Importante para segurança forense
             });
        }
  ...
  const calculateProgress = () => {
    ...
    // Etapa 1.5: Específicos de "Outro"
    if (formData.respondentType === 'other') {
        total += 2; // Especificação + Foco
        if (formData.respondentOtherSpec) filled++;
        if (formData.formFocus) filled++;
    }

    // Etapa 2: Questionários Específicos
    // Aqui usamos a lógica de qual fluxo está *ativo* para contar o total
    const isChildFlow = formData.respondentType === 'child' || formData.respondentType === 'teen' || formData.formFocus === 'child';
    const isParentFlow = formData.respondentType === 'parent' || formData.formFocus === 'parent';
    ...
    // Prevenir divisão por zero e garantir max 100
    if (total === 0) return 0;
                    ...
                    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="grid md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <input 
                                    type="text" 
                                    className={getInputClass(false)} // Nome nunca tem erro
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    onBlur={() => setAgeTouched(true)} // Só valida ao sair
                                />
                        ...
                        <div className={`p-5 rounded-lg border transition-all ${errors['respondentType'] ? 'border-[#cd3533] bg-red-50' : 'border-blue-100 bg-blue-50'}`}>
                            ...
                            {/* Lógica condicional para "OUTRO" */}
                            {formData.respondentType === 'other' && (
                                <div className="mt-4 pt-4 border-t border-blue-200 space-y-4 animate-in fade-in slide-in-from-top-2">
                                            ...
                                            {[
                                                ...
                                                >
                                                    <input 
                                                        ...
                                                        className="hidden" // Esconde o radio nativo para estilo customizado
                                                    />
                        ...
                        <button 
                            ...
                            className={`w-full py-4 text-lg font-bold text-white rounded-xl transition-all flex items-center justify-center gap-3 shadow-xl transform ${
                                ...
                                : Object.keys(errors).length > 0
                                    ? 'bg-[#cd3533] hover:bg-red-700 animate-pulse' // Visual de alerta se tiver erro ao tentar
